...
const sourcePath = path.join(projectRoot, "src", "cv_en.txt");
const outputDir = path.join(projectRoot, "exports");
const publicCvDir = path.join(projectRoot, "public", "cv");
const outputPdfPath = path.join(outputDir, "Supakorn_CV_EN_A4.pdf");
const publicPdfPath = path.join(publicCvDir, "Supakorn_CV_EN_A4.pdf");
const outputHtmlPath = path.join(outputDir, "Supakorn_CV_EN_A4.html");
...
function parseCvEnglish(rawText) {
  ...
  if (lines.length < 3) {
    throw new Error("cv_en.txt does not have enough content to build the document.");
  }
  ...
  const contactLines = data.contact.filter((line) => !line.startsWith("Portfolio:"));
  const portfolioLine = data.contact.find((line) => line.startsWith("Portfolio:")) || "";
  ...
  for (const line of data.experience) {
    ...
    if (!currentJob) {
      currentJob = { roleLine: "Additional Experience", details: [] };
    }
    ...
    <main class="page">
      <header class="header">
        ...
        <div class="meta">
          ${contactHtml}
          <div>
            Resume Online: <a href="${escapeHtml(resumeOnlineUrl)}" target="_blank" rel="noreferrer">${escapeHtml(resumeOnlineUrl)}</a>
          </div>
          <div class="portfolio-links">Portfolio: ${portfolioHtml}</div>
        </div>
      </header>
...
async function exportCvPdf() {
  ...
  const parsedCv = parseCvEnglish(rawText);
